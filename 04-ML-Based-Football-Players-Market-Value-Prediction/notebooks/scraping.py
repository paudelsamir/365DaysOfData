...
for page in pages:
    ...
    if table is not None: # table is the ovverall container of player data, holds all rows and columns
        ...
        for row in table.find_all("tr")[1:]: 
            ...
            if len(cols) > 0:  # Only process rows that have columns
                ...
                headers = [th.get_text().strip() for th in table.find("tr").find_all("th")]   # Extract the headers from the table (defines the strucuter of columns like Name, age , overall)
        ...
        print(f"Table not found on page: {page}") # this is just for time saving, i waited too long for the error before.
